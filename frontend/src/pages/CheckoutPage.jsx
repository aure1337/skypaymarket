...
function CheckoutPage() {
  ...
  async function fetchListing() {
    const { data } = await supabase.from('listings').select('*, profiles(id, username, slug, avatar_url, rating, sales_count)').eq('id', id).single()
    setListing(data)
    setLoading(false)
  }
  ...
  const handlePlaceOrder = async () => {
    ...
    await supabase.from('messages').insert([{
      ...
      content: `Пользователь оформил заказ "${listing.title}" за ${listing.price * quantity} ₽`,
      is_system: true
    }])

    // Редирект в чат
    navigate(`/chat/${orderData.id}`)
  }
    ...
    <div style={{ padding: '120px 0 40px' }}>
      <div className="container" style={{ maxWidth: '900px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 350px', gap: '24px' }}>
          <div>
            <div style={{ background: 'var(--card-solid)', padding: '24px', borderRadius: '16px', marginBottom: '20px', border: '1px solid var(--border)' }}>
